...
12. **Nunca una credencial real en un archivo de un repo — ni temporal, ni de prueba.** Ni siquiera en un repo privado, ni etiquetada "desechable". Si una verificación necesita crear/usar una credencial real, el valor se comunica fuera de git (chat) y en el repo se documenta solo el hecho, nunca el valor. Si ya se escribió por error: rotar la credencial real primero, luego reescribir el historial que la expone, y solo entonces reportar como cerrado. (Añadido 18 sep 2026, incidente real en este mismo repo — ver `ESTADO.md`.)
